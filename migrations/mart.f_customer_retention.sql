...
DELETE FROM mart.f_customer_retention WHERE period_id = {{ds}};
...
SELECT
    ...
    EXTRACT(week FROM uol.date_time) as period_id,
    uol.item_id,
...
FROM
    (SELECT COUNT(DISTINCT customer_id) as new_count, item_id, SUM(payment_amount) as new_revenue
     ...
     WHERE EXTRACT(week FROM uol.date_time) = {{ds}}
     GROUP BY item_id
...
JOIN
    (SELECT COUNT(DISTINCT customer_id) as refunded_count, item_id, SUM(payment_amount) as refunded_revenue
     FROM stage.user_order_log
     WHERE EXTRACT(week FROM uol.date_time) = {{ds}} AND action_id = 'refunded'
